...
def masked_loss_mean(raw_loss, boundary, loss, weight):
    """manually compute mean of raw_loss after applied a boundary mask"""
    # import pdb; pdb.set_trace()
    masked_loss = raw_loss * boundary
    masked_mean = masked_loss.sum(dim=1) / ((masked_loss != 0).sum(dim=1) + 1e-6)
    return loss(weight)(masked_mean)
...
def filter_dict(d, keys):
    """Returns dict with keys filtered."""
    return type(d)({key: d[key] for key in d if key not in keys})
...
class ExponentialDecaySchedule(Schedule):
    def __init__(self, config):
        """Exponential between max_p and min_p over schedule_timesteps."""
        super().__init__(config)
        self.max_p = self._hp.max_p
        self.min_p = self._hp.min_p
    ...
    def __call__(self, t):
        exponent = float(t) / self._hp.decay_steps
        return self.min_p + (self.max_p - self.min_p) * (self._hp.decay_factor ** exponent)
...
class MaxSizeQueue:
    """Creates a LIFO queue with a max size. Once an insert happens beyond that size, the oldest elem is removed and the new one inserted."""
